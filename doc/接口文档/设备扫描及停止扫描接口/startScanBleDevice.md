...
##### <span name="TTLockFromScan">扫描到的智能锁设备</span>  
 ...
 |------------------|-------------------|-----------------------|-----------|
 |deviceType        |number             |2.7.6 & offline-1.7.6  |[设备类型](../参数声明/设备通用参数.md#TTDEVICE_TYPE)|
 |type              |number             |2.7.0 & offline-1.7.0  |[智能锁类型](../参数声明/智能锁参数.md#TTLOCK_TYPE)|
 |deviceId          |string             |                       |蓝牙广播设备ID, 安卓设备与MAC地址相同，iOS为UUID格式|
 |rssi              |number             |                       |设备信号值, 0表示该设备已掉线|
 |isSettingMode     |boolean            |                       |设备是否处于可添加状态|
 |MAC               |string             |2.7.0 & offline-1.7.0  |蓝牙设备MAC地址|
 |deviceName        |string             |2.7.0 & offline-1.7.0  |设备蓝牙名称|
 |updatedTime       |number             |2.7.0 & offline-1.7.0  |设备扫描最后更新时间|
 |lockVersion       |TTLockVersion      |2.7.0 & offline-1.7.0  |[智能锁版本信息](#TTLockVersion), 3.1.0 & offline-2.1.0之前可通过[getLockVersion](../智能锁接口/getLockVersion.md)接口进行更新, 3.1.0 & offline-2.1.0开始可跳过该接口直接初始化|
 |electricQuantity  |number             |                       |智能锁设备电量|
 |isTouch           |boolean            |2.7.0 & offline-1.7.0  |设备是否处于可触摸开锁状态|  
...
##### <span name="TTLockVersion">智能锁版本信息</span>  
 ...
 |scene                 |number       |Y             |场景值|
 |groupId               |number       |N             |应用商ID, 3.1.0&offline-2.1.0之前扫描智能锁固定返回1, 3.1.0&offline-2.1.0开始扫描不再返回|
 |orgId                 |number       |N             |应用商子ID, 3.1.0&offline-2.1.0之前扫描智能锁固定返回1, 3.1.0&offline-2.1.0开始扫描不再返回|  

#### <span name="TTLockCallback">设备扫描失败回调</span>  
 3.0.8&offline-2.0.2及其以前：设备扫描失败回调  
 3.1.0&offline-2.1.0版本及其以上：设备扫描开启失败回调或未扫描到任意设备回调  
    1. 若因前者触发：触发时关闭蓝牙扫描  
