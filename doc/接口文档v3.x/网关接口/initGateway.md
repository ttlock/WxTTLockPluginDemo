...
): Promise<TTLockInitGatewayResult>
```  
<br />
...
## 参数说明 
 ...
 |----------------------|-----------------------------------------------------------|--------------|----------------|-----------|
 |option                |[TTLockInitGateway](#TTLockInitGateway)                    |Y             |IN              |初始化蓝牙网关全部参数|
 |deviceFromScan        |[TTGatewayFromScan](#TTGatewayFromScan)                    |Y             |IN              |扫描到的网关设备信息|
 |configuration         |[TTLockGatewayConfiguration](#TTLockGatewayConfiguration)  |Y             |IN              |初始化蓝牙网关配置信息|
 |disconnectCallback    |[TTLockCallback](#TTLockCallback)                          |N             |OUT             |设备断开连接回调|
<br />

### <span name="TTLockInitGateway">连接网关全部参数 TTLockInitGateway</span>  
 |PARAMS                |TYPE                                                       |REQUIRED      |IN/OUT          |DESCRIPTION|
 |----------------------|-----------------------------------------------------------|--------------|----------------|-----------|
 |deviceFromScan        |[TTGatewayFromScan](#TTGatewayFromScan)                    |Y             |IN              |扫描到的网关设备信息|
 |configuration         |[TTLockGatewayConfiguration](#TTLockGatewayConfiguration)  |Y             |IN              |初始化蓝牙网关配置信息|
...
### <span name="TTLockGatewayConfiguration">初始化蓝牙网关配置信息 TTLockGatewayConfiguration</span>  
 **网关服务器参数serverIPAddress与server参数至少配置一项，优先使用server**  
 通用网关服务器域名server: **plug.sciener.cn**  
 通用网关服务器端口port: **2999**  
 |NAME              |TYPE                               |REQUIRED       |DESCRIPTION|
 |------------------|-----------------------------------|---------------|-----------|
 |type              |[TTGATEWAY_TYPE](#TTGATEWAY_TYPE)  |Y              |网关类型|
 |uid               |number                             |Y              |通通锁用户ID，**非开发者账号**|
 |password          |string                             |Y              |通通锁用户密码，需MD5加密，32位|
 |companyId         |number                             |Y              |公司ID，没有则填0|
 |branchId          |number                             |Y              |分店ID, 没有则填0|
 |plugName          |string                             |Y              |网关命名，最长50个字符|
...
## 返回值
 异步返回操作回调结果 [TTLockInitGatewayResult](#TTLockInitGatewayResult)  
<br />

### <span name="TTLockInitGatewayResult">初始化蓝牙网关返回结果 TTLockInitGatewayResult extends TTLockError</span>  
 返回结果为[TTLockError](#TTLockError)的扩展，以下仅列出补充参数  
 |NAME                          |TYPE                                                           |VERSION    |DESCRIPTION|
 |------------------------------|---------------------------------------------------------------|-----------|-----------|
 |data                          |[TTLockInitGatewayResultData](#TTLockInitGatewayResultData)    |           |初始化蓝牙网关补充参数|
<br />

### <span name="TTLockInitGatewayResultData">蓝牙网关初始化补充参数 TTLockInitGatewayResultData</span>  
 |NAME                          |TYPE           |VERSION    |DESCRIPTION|
 |------------------------------|---------------|-----------|-----------|
