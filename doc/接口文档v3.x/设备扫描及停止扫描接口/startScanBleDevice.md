...
##### <span name="TTLockFromScan">扫描到的智能锁设备 TTLockFromScan</span>
 ...
 |lockVersion       |[TTLockVersion](#TTLockVersion)    |2.7.0      |智能锁版本信息, 扫描时groupID和orgId固定返回1, 可通过[getLockVersion](../智能锁接口/getLockVersion.md)接口进行更新|
...
### <span name="TTLockCallback">TTLockCallback 设备扫描失败回调</span>  
```
(res: TTLockError) => any
```  
#### 回调方法参数说明
 |PARAMS                    |TYPE                           |IN/OUT         |DESCRIPTION|
 |--------------------------|-------------------------------|---------------|-----------|
 |res                       |[TTLockError](#TTLockError)    |OUT            |设备扫描失败结果及相关说明|
<br />
...
### <span name="TTLockError">常规错误返回结果 TTLockError</span>  
 ...
 |electricQuantity              |boolean                    |2.7.0      |智能锁设备电量|
<br />
...
## <span name="TTLOCK_TYPE">TTLOCK_TYPE 智能锁类型</span>  
 [更多信息](../参数声明/智能锁参数.md#TTLOCK_TYPE)  
 |VALUE         |VERSION    |DESCRIPTION|
 |--------------|-----------|-----------|
 |5             |           |三代锁(小程序支持版本)|
 |4             |           |二代锁场景二(小程序不支持蓝牙操作)|
