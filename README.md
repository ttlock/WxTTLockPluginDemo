...
```
    interface TTLock.ScanDevice {
        deviceType: DEVICE_TYPE; // 设备类型 (2.7.6新增)
        type: LOCK_TYPE; // 智能锁类型(2.7.0新增)
        ...
        lockVersion: TTLock.LockVersion; // 智能锁版本信息, 扫描时groupId和orgId返回固定值1, 需通过getLockVersion接口更新 (2.7.0新增)
...
`function getLockType(lockVersion: TTLock.LockVersion): LOCK_TYPE`  

###### 参数
+ lockVersion：[智能锁版本信息](#1-智能锁版本信息) 

###### 返回值 
+ 智能锁类型，请参考[LOCK_TYPE](#3-智能锁类型)参数说明
...
`function parseSpecialValues(featureValue:string | number): TTLock.LockFeatureValue`
...
+ 说明：
```
    interface TTLock.LockFeatureValue {
